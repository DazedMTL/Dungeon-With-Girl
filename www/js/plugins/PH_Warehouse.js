...
PHPlugins.Params.PHWarehouseWithdrawText = String(
  PHPlugins.Parameters["Withdraw Text"]
);
PHPlugins.Params.PHWarehouseDepositText = String(
  PHPlugins.Parameters["Deposit Text"]
);
PHPlugins.Params.PHWarehouseAvailableSpaceText = String(
  PHPlugins.Parameters["Available Space Text"]
);
PHPlugins.Params.PHWarehouseAllText = String(PHPlugins.Parameters["All Text"]);
PHPlugins.Params.PHWarehouseAllTogether =
  Number(PHPlugins.Parameters["All Together"]) || 0;
PHPlugins.Params.PHWarehouseStackItemQuantity =
  Number(PHPlugins.Parameters["Stack Item Quantity"]) || 0;
PHPlugins.Params.PHWarehouseAllTogether = Boolean(
  PHPlugins.Params.PHWarehouseAllTogether
);
PHPlugins.Params.PHWarehouseStackItemQuantity = Boolean(
  PHPlugins.Params.PHWarehouseStackItemQuantity
);
...
(function () {
      ...
      if (!this._warehouses.hasOwnProperty(title)) {
        if (results.length >= 2) {
          ...
          if (
            ...
            this._rules.hasOwnProperty(results[2])
          ) {
            rule = results[2];
          }
        }
  ...
  PHWarehouseManager.prototype.addLoot = function (_sentence, category) {
    ...
    if (matches != null) {
      results = matches.split(":");
      if (
        this._warehouses.hasOwnProperty(results[0]) &&
        this._warehouses[results[0]].lootBonus &&
        ...
        if (
          results[2] >
          this._warehouses[results[0]].maxCapacity -
            this._warehouses[results[0]].currentCapacity
        ) {
          results[2] =
            this._warehouses[results[0]].maxCapacity -
            this._warehouses[results[0]].currentCapacity;
        ...
        if (
          this._warehouses[results[0]].items[category].indexOf(results[1]) > -1
        ) {
  ...
  PHWarehouseManager.prototype.loadRules = function () {
    ...
    if ($dataCommonEvents) {
      for (var i = 0; i < $dataCommonEvents.length; i++) {
        if (
          $dataCommonEvents[i] instanceof Object &&
          $dataCommonEvents[i].name == "PHWarehouse"
        ) {
        ...
        if (this.checkTitle(str)) {
          ...
          if (rule[0].indexOf("-n") > -1) {
            ...
            if (this._rules[index].disabledItems.hasOwnProperty(rule[0])) {
              this._rules[index].disabledItems[rule[0]] = this.getItemsId(
                rule[1]
              );
            }
          } else {
            if (this._rules[index].enabledItems.hasOwnProperty(rule[0])) {
              if (rule[1].indexOf("no") > -1) {
                ...
                this._rules[index].enabledItems[rule[0]] = this.getItemsId(
                  rule[1]
                );
              }
            }
  ...
  PHWarehouseManager.prototype.isItemEnabled = function () {
    if (
      this._warehouses[this._lastActive].rule == null ||
      (this._rules.hasOwnProperty(this._warehouses[this._lastActive].rule) &&
        Array.isArray(
          this._rules[this._warehouses[this._lastActive].rule].enabledItems.item
        ))
    ) {
      return true;
    }
  ...
  PHWarehouseManager.prototype.isWeaponEnabled = function () {
    if (
      ...
      (this._rules.hasOwnProperty(this._warehouses[this._lastActive].rule) &&
        Array.isArray(
          this._rules[this._warehouses[this._lastActive].rule].enabledItems
            .weapon
        ))
    ) {
      return true;
  ...
  PHWarehouseManager.prototype.verifyItem = function (item) {
    ...
    if (
      ...
      if (
        this._warehouses[this._lastActive].rule !== null &&
        Array.isArray(
          this._rules[this._warehouses[this._lastActive].rule].disabledItems[
            this._lastCategory
          ]
        ) &&
        this._rules[this._warehouses[this._lastActive].rule].disabledItems[
  ...
  PHWarehouseManager.prototype.setMaxCapacity = function (_sentence) {
    ...
    if (matches != null) {
      ...
      if (results.length == 2) {
        ...
        if (
          this._warehouses.hasOwnProperty(title) &&
          !isNaN(capacity) &&
          capacity >= this.getCurrentCapacity(title)
        ) {
  ...
  PHWarehouseManager.prototype.getWeapons = function () {
    ...
    for (
      var i = 0;
      i < this._warehouses[this._lastActive].items.weapon.length;
      i++
    ) {
      for (var j = 0; j < $dataWeapons.length; j++) {
        if (
          $dataWeapons[j] != null &&
          this._warehouses[this._lastActive].items.weapon[i] ==
            $dataWeapons[j].id
        ) {
  ...
  PHWarehouseManager.prototype.getCommonItems = function () {
    ...
    for (
      var i = 0;
      i < this._warehouses[this._lastActive].items.item.length;
      i++
    ) {
      for (var j = 0; j < $dataItems.length; j++) {
        if (
          $dataItems[j] != null &&
          this._warehouses[this._lastActive].items.item[i] == $dataItems[j].id
        ) {
  ...
  PHWarehouseManager.prototype.getArmors = function () {
    ...
    for (
      var i = 0;
      i < this._warehouses[this._lastActive].items.armor.length;
      i++
    ) {
      for (var j = 0; j < $dataArmors.length; j++) {
        if (
          $dataArmors[j] != null &&
          this._warehouses[this._lastActive].items.armor[i] == $dataArmors[j].id
        ) {
  ...
  PHWarehouseManager.prototype.getKeyItems = function () {
    ...
    for (
      var i = 0;
      i < this._warehouses[this._lastActive].items.keyItem.length;
      i++
    ) {
      for (var j = 0; j < $dataItems.length; j++) {
        if (
          $dataItems[j] != null &&
          this._warehouses[this._lastActive].items.keyItem[i] ==
            $dataItems[j].id
        ) {
          totalItems.push($dataItems[j]);
        }
  ...
  PHWarehouseManager.prototype.deposit = function (item) {
    if (this.checkCapacity()) {
      ...
      if (this._lastCategory != "all") {
        var hasItem = false;
        if (
          this._warehouses[this._lastActive].items[this._lastCategory].indexOf(
            item.id
          ) > -1
        ) {
          hasItem = true;
        ...
        if (hasItem) {
          this._warehouses[this._lastActive].qtty[this._lastCategory][
            item.id
          ]++;
        } else {
          this._warehouses[this._lastActive].items[this._lastCategory].push(
            item.id
          );
          this._warehouses[this._lastActive].qtty[this._lastCategory][
            item.id
  ...
  PHWarehouseManager.prototype.withdraw = function (item) {
    ...
    if (this._lastCategory != "all") {
      var hasItem = false;
      var index = this._warehouses[this._lastActive].items[
        this._lastCategory
      ].indexOf(item.id);
      if (index > -1) {
        hasItem = true;
      ...
      if (hasItem) {
        ...
        if (
          ...
          this._warehouses[this._lastActive].items[this._lastCategory].splice(
            index,
            1
          );
          delete this._warehouses[this._lastActive].qtty[this._lastCategory][
            item.id
          ];
        }
        this._warehouses[this._lastActive].currentCapacity--;
  ...
  PHWarehouseManager.prototype.hasItems = function (title, id, category) {
    if (
      this._warehouses.hasOwnProperty(title) &&
      this._warehouses[title].items[category].indexOf(id) > -1
    ) {
  ...
  PHWarehouseManager.prototype.getCurrentCapacity = function (title) {
    if (this._warehouses.hasOwnProperty(title)) {
      if (PHPlugins.Params.PHWarehouseStackItemQuantity == true) {
        return (
          this._warehouses[title].items.item.length +
          this._warehouses[title].items.weapon.length +
          this._warehouses[title].items.keyItem.length +
          this._warehouses[title].items.armor.length
        );
      } else {
  ...
  PHWarehouseManager.prototype.exist = function (title) {
    if (
      this._warehouses.hasOwnProperty(title) &&
      this._warehouses[title] !== undefined
    ) {
      return true;
    }
    return false;
    ...
    if (command === "PHWarehouse") {
      switch (args[0]) {
        ...
        case "add":
          switch (args[1]) {
            case "item":
              ...
              break;
            case "weapon":
              PHPlugins.PHWarehouse.addItems(
                getAllArguments(args, 2),
                "weapon"
              );
              break;
            case "armor":
        ...
        case "capacity":
          switch (args[1]) {
            case "increase":
              PHPlugins.PHWarehouse.increaseMaxCapacity(
                getAllArguments(args, 2)
              );
              break;
            case "decrease":
              PHPlugins.PHWarehouse.decreaseMaxCapacity(
                getAllArguments(args, 2)
              );
              break;
            case "set":
  ...
  Window_WarehouseTitle.prototype.refresh = function () {
    ...
    this.drawText(
      PHPlugins.PHWarehouse._lastActive,
      0,
      0,
      Graphics.boxWidth,
      "center"
  ...
  Window_WarehouseCategory.prototype = Object.create(
  ...
  Window_WarehouseItemList.prototype.initialize = function () {
    Window_ItemList.prototype.initialize.call(
      this,
      0,
      this.fittingHeight(5),
      Graphics.boxWidth,
      Graphics.boxHeight - this.fittingHeight(7)
    );
  };
  ...
  Window_WarehouseItemList.prototype.isCurrentItemEnabled = function () {
    if (this._data.length > 0) {
      if (
        PHPlugins.PHWarehouse._lastOption == 1 &&
        PHPlugins.PHWarehouse.checkCapacity()
      ) {
        return true;
  ...
  Window_WarehouseItemList.prototype.drawWarehouseItemNumber = function (
    item,
    x,
    y,
    width
  ...
  Window_WarehouseInfo.prototype.initialize = function () {
    Window_Base.prototype.initialize.call(
      this,
      0,
      Graphics.boxHeight - this.fittingHeight(1),
      Graphics.boxWidth,
      this.fittingHeight(1)
    );
    this.availableSpaceText =
      PHPlugins.Params.PHWarehouseAvailableSpaceText + " ";
    this.refresh();
  };

  Window_WarehouseInfo.prototype.refresh = function () {
    this.contents.clear();
    this.availableSpaceValue =
      PHPlugins.PHWarehouse._warehouses[PHPlugins.PHWarehouse._lastActive]
        .maxCapacity -
      PHPlugins.PHWarehouse.getCurrentCapacity(
        PHPlugins.PHWarehouse._lastActive
      ) +
      " / " +
      PHPlugins.PHWarehouse._warehouses[PHPlugins.PHWarehouse._lastActive]
    ...
    this.drawText(
      this.availableSpaceText + this.availableSpaceValue,
      0,
      0,
      this.x
    );
