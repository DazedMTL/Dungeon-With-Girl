...
Scene_Title.prototype.update = function () {
  ...
  if (!CBR_eroStaNPC.data && this.isActive()) {
    ...
    if (Number(param["テキストのサイズ"]) === 0) {
      ...
      CBR_eroStaNPC.param["txtサイズ"] =
        Number(param["テキストのサイズ"]) || 26;
    }
    ...
    if (Number(param["ページ移動のSEの音量"]) === 0) {
      ...
      CBR_eroStaNPC.param["se音量"] =
        Number(param["ページ移動のSEの音量"]) || 100;
    }
    if (Number(param["ページ移動のSEのピッチ"]) === 0) {
      CBR_eroStaNPC.param["seピッチ"] = 0;
    } else {
      CBR_eroStaNPC.param["seピッチ"] =
        Number(param["ページ移動のSEのピッチ"]) || 100;
    }
...
CBR_eroStaNPC.addTxt = function (ary) {
  ...
  if (!tempD["原点"]) {
    tempD["原点"] = this.param["txt原点"];
  }
  if (!tempD["揃え"]) {
    tempD["揃え"] = this.param["txt揃え"];
  }
  if (!tempD["サイズ"]) {
    tempD["サイズ"] = this.param["txtサイズ"];
  }
...
Window_EroStaNPC.prototype.initialize = function () {
  //windowを作る
...
Window_EroStaNPC.prototype.CBR_setPage = function (val) {
  //現在のページ
  this._CBR_page = val;
};
...
Window_EroStaNPC.prototype.CBR_setComplete = function (val) {
  //描写終わったかどうか
...
Window_EroStaNPC.prototype.update = function () {
  ...
  if (2 < CBR_eroStaNPC.data.length && this.isOpenAndActive()) {
    //2ページ以上存在して操作があった時
    if (Input.isRepeated("right") || Input.isRepeated("left")) {
      AudioManager.playSe({
        name: CBR_eroStaNPC.param["se名"],
        volume: CBR_eroStaNPC.param["se音量"],
        pitch: CBR_eroStaNPC.param["seピッチ"],
        pan: CBR_eroStaNPC.param["se位相"],
      });
  ...
  if (!this.CBR_complete()) {
    ...
    for (var i = 0, len = ary.length; i < len; i++) {
      ...
      if (name) {
        ...
        if (temp) {
          if (!temp.width) {
            //暗号化するとwidthでしか判別できない　探せばあるかもしれないけど
...
Window_EroStaNPC.prototype.drawPicture = function (
  dir,
  filename,
  sx,
  sy,
...
var _CBR_Game_Interpreter_pluginCommand =
