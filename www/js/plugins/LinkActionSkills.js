...
(function () {
  ...
  BattleManager.startAction = function () {
    ...
    for (var i = 0; i < this._targets.length; i++) {
      ...
      var counterlist = this._targets[i].calcSkillCounter(
        this._subject._lastActionLS,
        this.counterStartSection,
        i
      );
    ...
    if (this.exActionListCrush.length > 0) {
      ...
      if (
        this._subject &&
        (this._subject.canMove() || nextAction.counter_ignorebind)
      ) {
        //取り消し
        this._logWindow.counterInterrupt(
          subject,
          this._subject,
          subject._lastActionLS,
          nextAction
        );
  ...
  Window_BattleLog.prototype.counterInterrupt = function (
    origSubj,
    newSubj,
    oldAction,
    newAction
  ...
  Game_Enemy.prototype.index = function () {
    ...
    if (this.backupIndex === 0 || this.backupIndex) {
  ...
  BattleManager.endAction = function () {
    ...
    for (var i = 0; i < this.prevTargets.length; i++) {
      var counterlist = this.prevTargets[i].calcSkillCounter(
        this._subject._lastActionLS,
        this.counterStartSection,
        i
      );
  ...
  BattleManager.generateLinkedAction = function () {
    ...
    for (var i = 0; i < this.prevTargets.length; i++) {
      ...
      if (singleResult.isHit()) {
        someoneHit = true;
      }
      if (!singleResult.isHit()) {
        someoneEvade = true;
      }
      if (singleResult.critical) {
        someoneCrit = true;
      }
    }
    if (nextaction.item().meta.counteronhit && !someoneHit) {
      return null;
    }
    if (nextaction.item().meta.counteronevade && !someoneEvade) {
      return null;
    }
    if (nextaction.item().meta.counteroncrit && !someoneCrit) {
      return null;
    }
    ...
    if (
      nextaction.item().meta.linkcondition &&
      !eval(nextaction.item().meta.linkcondition)
    ) {
      return null;
    }

    if (!nextaction || !nextaction.item() || !nextaction.item().meta.linkskill)
      return null;
    if (
      !(
  ...
  //-----------------------Counters---------------------------------------------
  var Game_Battler_prototype_endTurnAllCTB =
    Game_Battler.prototype.endTurnAllCTB;
  ...
  var Game_BattlerBase_prototype_updateStateActionEnd =
    Game_BattlerBase.prototype.updateStateActionEnd;
  ...
  var Game_BattlerBase_prototype_updateStateTurns =
  ...
  Game_BattlerBase.prototype.calcSkillCounter = function (
    action,
    counterStartSection,
    index
  ) {
    ...
    for (var i = 0; i < applicableStateList.length; i++) {
      ...
      if (state) {
        ...
        if (state.meta.counteronhit && !result.isHit()) {
          continue;
        }
        if (state.meta.counteronevade && result.isHit()) {
          continue;
        }
        if (state.meta.counteroncrit && !result.critical) {
          continue;
        }

        if (state.meta.linkcondition && !eval(state.meta.linkcondition)) {
          continue;
        }

        if (state.meta.statecounter) {
          var meta_Array = state.metaArray
            ? state.metaArray.statecounter
            : [state.meta.statecounter];
          ...
          meta_Array.some(function (stateWhole) {
            ...
            if (stateC.length >= 3) {
              ...
              var counterStateSkill = isReflect
                ? action.item().id
                : Number(stateC[2]);

              if (
                this._lastAddedState.indexOf(counterState) > -1 &&
                Math.random() * 100 < counterStateRate
              ) {
                var newaction = new Game_Action(
                  applicableStateList[i].target,
                  true
                );
        ...
        if (state.meta.elementcounter) {
          var meta_Array = state.metaArray
            ? state.metaArray.elementcounter
            : [state.meta.elementcounter];
          var incoming_isSkill = action.isSkill();
          ...
          meta_Array.some(function (elementWhole) {
            ...
            if (elementC.length >= 3) {
              ...
              var counterSkill = isReflect
                ? action.item().id
                : Number(elementC[2]);

              if (
                extraElements.indexOf(Number(counterElement)) >= 0 &&
                Math.random() * 100 < counterRate
              ) {
                var newaction = new Game_Action(
                  applicableStateList[i].target,
                  true
  ...
  var kz_Game_Battler_prototype_removeState =
