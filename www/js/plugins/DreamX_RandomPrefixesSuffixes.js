...
(function () {
  ...
  var paramMultiplier = parseInt(
    parameters["Bonus Parameter Price Multiplier"] || 10
  );
  var paramEditOld = eval(
    parameters["Edit Database For Old Notetags"] || false
  );
  var paramAlwaysChoose = eval(
    parameters["Always Choose Prefix/Suffix"] || true
  );
  var paramCombIconStarting = parseInt(
    parameters["Icon Combination Starting Index"] || 6000
  );
  ...
  DataManager.loadDatabase = function () {
    ...
    if (!Imported.YEP_ItemCore) {
      throw new Error(
        "DreamX_RandomPrefixesSuffixes requires Yanfly Item Core"
      );
    }
    var independentItemCount =
      Yanfly.Param.ItemMaxItems +
      Yanfly.Param.ItemMaxWeapons +
      Yanfly.Param.ItemMaxArmors;
    if (independentItemCount <= 0) {
      throw new Error(
        "DreamX_RandomPrefixesSuffixes requires Yanfly " +
          "Item Core to have independent items enabled."
      );
    }
  };

  DreamX.RandomPrefixSuffix.DataManager_isDatabaseLoaded =
  ...
  DataManager.processDXRPSNotetagsAugmentChoices = function (element) {
    ...
    for (var i = 0; i < augmentChoiceLines.length; i++) {
      ...
      element.DXRPSAugmentChoices.push({
  ...
  DataManager.processDXRPSNotetagsChoices = function (element) {
    ...
    for (var i = 0; i < choiceNoteLines.length; i++) {
      ...
      element.DXRPSAffixChoices.push({
        lineChoices: lineChoices,
        affixType: affixType,
      });
      lineChoices = [];
  ...
  DataManager.editRandomPrefixSuffixMetaNotes = function (dataArray) {
    for (var i = 0; i < dataArray.length; i++) {
      ...
      element.note.replace(
        "DreamX.RandomPrefixAffix",
        "DreamX.RandomPrefixSuffix"
      );
    }
  };

  DreamX.RandomPrefixSuffix.evaluateCustomEffect = function (
    item,
    bItem,
    nItem
  ) {
  ...
  DreamX.RandomPrefixSuffix.makeChoices = function (arrayOfIDs, dataType) {
    ...
    for (var i = 0; i < arrayOfIDs.length; i++) {
      var percent = dataType[arrayOfIDs[i]].meta.prefixSuffixChance
        ? dataType[arrayOfIDs[i]].meta.prefixSuffixChance
        : paramDefaultChance;
  ...
  DreamX.RandomPrefixSuffix.combineWithArmorWeapon = function (
    prefixSuffixItem,
    newItem
  ) {
    ...
    if (prefixSuffixItem.meta.prefixSuffixParameters) {
      this.randomizeBonusParameters(
        prefixSuffixItem.meta.prefixSuffixParameters,
        newItem
      );
    }
  ...
  DreamX.RandomPrefixSuffix.combineWithRegularItem = function (
    prefixSuffixItem,
    newItem
  ) {
    // combinations
  ...
  DreamX.RandomPrefixSuffix.randomizeBonusParameters = function (
    notetag,
    newItem
  ) {
    ...
    while (i < parameterSplit.length) {
      var parameterID = DreamX.RandomPrefixSuffix.interpretParamNote(
        parameterSplit[i]
      );
      i++;
  ...
  DreamX.RandomPrefixSuffix.ChoosePrefixSuffixItem = function (
    itemMeta,
    dataType
  ) {
    var itemArray = [];

    var itemSplit = itemMeta
      .trim()
      .replace(/,/g, " ")
      .split(new RegExp("\\s{1,}"));
  ...
  DreamX.RandomPrefixSuffix.combineWithBaseItem = function (
    prefixSuffixItem,
    newItem,
    itemType
  ) {
    var prefixSuffixName = prefixSuffixItem.meta.prefixSuffixName
      ? prefixSuffixItem.meta.prefixSuffixName
      : prefixSuffixItem.name;
    ...
    newItem.DXRPSAugmentChoices = newItem.DXRPSAugmentChoices.concat(
      prefixSuffixItem.DXRPSAugmentChoices
    );
  ...
  DreamX.RandomPrefixSuffix.addIconOverlay = function (source, dest) {
    ...
    dest.overlayIcons.push({
      index: source.iconIndex,
  ...
  DreamX.RandomPrefixSuffix.makeItem = function (baseItem) {
    ...
    if (newItem.meta.prefix) {
      var prefixItem = this.ChoosePrefixSuffixItem(
        newItem.meta.prefix,
        dataType
      );
    ...
    if (newItem.meta.suffix) {
      var suffixItem = this.ChoosePrefixSuffixItem(
        newItem.meta.suffix,
        dataType
      );
    ...
    if (newItem._DXHighestParamBonus && newItem._DXHighestParamBonus !== 0) {
      ...
      var bonusParamText = paramBonusParamText.format(
        sign,
        newItem._DXHighestParamBonus
      );
    ...
    if (newItem.overlayIcons) {
      ...
      var newCombIconId =
        paramCombIconStarting + $gameSystem.overlayIcons.length;
    ...
    //DataManager.registerNewItem(newItem);  //←ItemCoreと重複の為コメントアウト

    //newItem = JSON.parse(JSON.stringify($gameTemp.lastDXRPSItemCreated)); //←ItemCoreと重複による影響の為コメントアウト
    newItem.DXRPSNewItem = true;

    return newItem;
  };

  DreamX.RandomPrefixSuffix.GainPrefixSuffixItem = function (
    item,
    amount,
    includeEquip
  ) {
    for (var i = 0; i < amount; i++) {
      $gameParty.gainItem(
        DreamX.RandomPrefixSuffix.makeItem(item),
        1,
        includeEquip
      );
    }
  ...
  Game_Party.prototype.gainItem = function (item, amount, includeEquip) {
    ...
    if (
      amount > 0 &&
      DreamX.RandomPrefixSuffix.isConfiguredForPrefixSuffix(item)
    ) {
      DreamX.RandomPrefixSuffix.GainPrefixSuffixItem(
        item,
        amount,
        includeEquip
      );
    } else {
      DreamX.RandomPrefixSuffix.Game_Party_gainItem.call(
        this,
        item,
        amount,
        includeEquip
  ...
  Game_Party.prototype.getMatchingBaseItem = function (baseItem, equipped) {
    ...
    for (var i = 0; i < group.length; ++i) {
      ...
      if (
        ...
        item.originalBaseItemId !== baseItemId
      ) {
        continue;
      }
  ...
  DreamX.RandomPrefixSuffix.isConfiguredForPrefixSuffix = function (item) {
    if (!item || item.DXRPSItem) return false;

    if (
      !item.meta.suffix &&
      !item.meta.prefix &&
      item.DXRPSAffixChoices.length <= 0
    )
      return false;
  ...
  DreamX.RandomPrefixSuffix.DataManager_isNewItemValid =
    DataManager.isNewItemValid;
  DataManager.isNewItemValid = function (item) {
    if (this.isDXRPSNewItem(item)) {
      return true;
    }
    return DreamX.RandomPrefixSuffix.DataManager_isNewItemValid.call(
      this,
      item
    );
  };
  ...
  Game_Party.prototype.numIndependentItems = function (baseItem) {
    ...
    for (var i = 0; i < group.length; ++i) {
      ...
      if (
        (item.baseItemId && item.baseItemId === id) ||
  ...
  DreamX.RandomPrefixSuffix.chooseAugments = function (newItem) {
    ...
    for (var i = 0; i < newItem.DXRPSAugmentChoices.length; i++) {
      ...
      newItem.DXRPSChosenAugments.push({
        id: id,
        dataType: dataType,
        augmentType: augmentType,
      });
  ...
  DreamX.RandomPrefixSuffix.DataManager_addNewIndependentItem =
    DataManager.addNewIndependentItem;
  DataManager.addNewIndependentItem = function (baseItem, newItem) {
    var originalNote = newItem.note;
    DreamX.RandomPrefixSuffix.DataManager_addNewIndependentItem.call(
      this,
      baseItem,
      newItem
    );
    if (!this.isDXRPSItem(newItem)) return;
  ...
  DreamX.RandomPrefixSuffix.Window_ItemStatus_drawLargeIcon =
    Window_ItemStatus.prototype.drawLargeIcon;
  Window_ItemStatus.prototype.drawLargeIcon = function () {
    var iconIndex = this._item.iconIndex;
    if (iconIndex < paramCombIconStarting) {
      return DreamX.RandomPrefixSuffix.Window_ItemStatus_drawLargeIcon.call(
        this
      );
    }
  ...
  // Window_Base
  //==========================================================================
  DreamX.RandomPrefixSuffix.Window_Base_drawIcon =
  ...
  if (Imported.YEP_VictoryAftermath) {
    DreamX.RandomPrefixSuffix.BattleManager_gainDropItems =
    ...
    DreamX.RandomPrefixSuffix.BattleManager_displayDropItems =
  ...
  if (Imported.YEP_ShopMenuCore) {
    DreamX.RandomPrefixSuffix.Scene_Shop_doBuyItem =
      Scene_Shop.prototype.doBuyItem;
    Scene_Shop.prototype.doBuyItem = function (number) {
      var item = this._item;
      if (!item || !item.DXRPSDummyShopItem) {
        return DreamX.RandomPrefixSuffix.Scene_Shop_doBuyItem.call(
          this,
          number
        );
      }
    ...
    DreamX.RandomPrefixSuffix.Window_ShopInfo_drawLargeIcon =
      Window_ShopInfo.prototype.drawLargeIcon;
    Window_ShopInfo.prototype.drawLargeIcon = function () {
      var iconIndex = this._item.iconIndex;
      if (iconIndex < paramCombIconStarting) {
        return DreamX.RandomPrefixSuffix.Window_ShopInfo_drawLargeIcon.call(
          this
        );
      }
      this.DXDrawLargeIcon();
  ...
  if (Imported.YEP_X_ItemDurability) {
    DreamX.RandomPrefixSuffix.ItemManager_makeDurability =
      ItemManager.makeDurability;
    ItemManager.makeDurability = function (item, variance) {
      if (!item || (item.DXRPSItem && item.id === item.baseItemId)) {
        return;
      }
      DreamX.RandomPrefixSuffix.ItemManager_makeDurability.call(
        this,
        item,
        variance
      );
  ...
  if (Imported.ShopManager) {
    ...
    DreamX.RandomPrefixSuffix.Game_Shop_setupGoods =
